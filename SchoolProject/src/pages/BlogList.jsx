...
const BlogList = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await axios.get('http://localhost:9090/api/public/getBlogs', {
          headers: {
  ...
  const handleImageError = (e, title) => {
    // console.error(`Failed to load image for blog: ${title}`, e);
    ...
    <div className='bg-[linear-gradient(135deg,_#e0cff2,_#d7e2f5)]'>
      ...
      <div className="flex-grow overflow-y-auto">
        {loading ? (
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4 md:gap-6">
            {blogs.map((blog) => (
              <div
                key={blog.id}
              ...
              >
                ...
                <div className="p-2 sm:p-3 md:p-4">
                  ...
                  <p className="text-gray-600 text-xs sm:text-sm md:text-base mb-1 sm:mb-2">By {blog.author}</p>
                  <p className="text-gray-700 text-xs sm:text-sm md:text-base line-clamp-3">{blog.content}</p>
                </div>
              </div>
            ))}
          </div>
